...
def process_app_file(source_dir, source_files, data_dir, out_file, env):
    ...
    env.Depends("$BUILDSRC_DIR/esp-rfid.ino.o", env.Command(header_file, data_file, data_to_header))
...
def process_html_app(source, dest, env):
    ...
    env.Depends("$BUILDSRC_DIR/esp-rfid.ino.o", env.Command(header_file, manifest, make_static))
